...
def f(x,mu):
    ...
    if y < error:
        return (9 - 8*x[0] - 6*x[1] - 4*x[2] + 2*x[0]**2 + 2*x[1]**2 + x[2]**2 +
            2*x[0]*x[1] + 2*x[0]*x[2] -mu*np.log(x[0] +x[1] + 2*x[2] -3)-
            mu*np.log(x[0])-mu*np.log(x[1])-mu*np.log(x[2]))
    else:
                return (9 - 8*x[0] - 6*x[1] - 4*x[2] + 2*x[0]**2 + 2*x[1]**2 + x[2]**2 +
            2*x[0]*x[1] + 2*x[0]*x[2] -4*mu*np.log(error))
def fun(x):
...
def grad(x,mu):
    ...
    y = x[0]+x[1]+2*x[2]-3
    if y < -error:
        return np.array([ -8+4*x[0]+2*x[1]+2*x[2] -mu*1/(x[0]+x[1]+2*x[2]-3)-mu/(x[0]),
                    -6+4*x[1]+2*x[0] -mu*1/(x[0]+x[1]+2*x[2]-3)-mu/(x[1]) ,
                    -4+2*x[2]+2*x[0] -2*mu*1/(x[0]+x[1]+2*x[2]-3)-mu/(x[2])])
    else:
...
def BFGS(f,x0,max_it,fun):
    ...
    while np.linalg.norm(ngrad) > 1e-4: # condicion de parada
        ...
        print("n:", it, " | P(x) =", f(x, mu), " | f(x) =", fun(x), "| Mu: ",mu, "| x: ", x  )
    return x
# resultado
x_opt = BFGS(f,[0.3,0.5,0.4],9,fun)
